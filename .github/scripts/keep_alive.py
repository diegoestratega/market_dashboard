...
def page_text(page) -> str:
    """Visible text from every frame, not just the top document.

    Community Cloud serves the app inside an <iframe title="streamlitApp">;
    the outer document holds only the hosting chrome ("Hosted with Streamlit",
    roughly 30 characters) and the dashboard itself lives one frame down. A
    local `streamlit run` has no such wrapper, so reading only the main frame
    passes locally and reports an empty page against the real deployment.
    """
    parts = []
    for frame in page.frames:
        try:
            parts.append(frame.locator("body").inner_text(timeout=5_000))
...
def click_wake_button(page) -> bool:
    ...
    Searched per frame for the same reason as page_text: the interstitial may
    render inside the app frame rather than the outer hosting document.
    """
    for frame in page.frames:
        for role in ("button", "link"):
            try:
                control = frame.get_by_role(role, name=WAKE_BUTTON).first
                if control.count() > 0:
                    control.click(timeout=10_000)
                    return True
            except (PlaywrightTimeout, PlaywrightError):
                continue
        # Fall back to a raw text match if it is neither a button nor a link.
        try:
            control = frame.get_by_text(WAKE_BUTTON).first
            if control.count() > 0:
                control.click(timeout=10_000)
                return True
        except (PlaywrightTimeout, PlaywrightError):
